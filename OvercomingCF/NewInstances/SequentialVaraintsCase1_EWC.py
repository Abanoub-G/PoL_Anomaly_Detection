...
#datagen = ImageDataGenerator(featurewise_center=True,rotation_range=90,horizontal_flip=True, vertical_flip=True)
...
#np.save('X_train_1.npy', X_train) # save
#np.save('Y_new_1.npy', Y_new) # save

X_train_1 = np.load('X_train_1.npy') # load
Y_new_1 = np.load('Y_new_1.npy') # load
...
for i in range(10):
 ...
 task_temp_1 = [(X_train_1[0+50*i:100+50*i], Y_new_1[0+50*i:100+50*i]), (X_train_1[0+50*i:100+50*i], Y_new_1[0+50*i:100+50*i])]
 tasks.append(task_temp_1)
...
def train_ewc(model, device, task_id, x_train, t_train, optimizer, epoch, EWC_ON, ewc_lambda,batch):
    model.train()
    
    #batch size is 100
    for start in range(0, len(t_train)-1, batch):
      end = start + batch
...
for epoch in range(0, 10):
   model = train_ewc(model, device, 0, x_train, t_train, optimizer, epoch, EWC_ON,0,64)
...
# copyed_model = copy.deepcopy(model)
 ...
 for epoch in range(0, e):   
  copyed_model = train_ewc(copyed_model, device, id_, x_train, t_train, optimizer, epoch,True,ewc_lambda,b)
...
for i in range(0,20):
 ...
 for epoch in range(0, E_list[0]):
   model = train_ewc(model, device, i+1, x_train, t_train, optimizer, epoch, EWC_ON,1,B_list[1])
...
with open('NewInstanceCase1_lr0.01_E5_B16_EWC_EachSet100', 'wb') as fp:
