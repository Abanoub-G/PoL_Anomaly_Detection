...
if Retraining_flag == True :
	for retraining_set in retraining_sets:
		...
		if acc < Misclassification_threshold:
			...
			acc_train_init_orig =i5= test(trained_models[-2], device, X_train, Y_train) # Evaluate model on initial training data
			# print("5) Accuracy on initial training data (X_train, Y_train)", acc_train_init_orig)
			print("5) ", acc_train_init_orig)

			acc_test_init_orig =i6= test(trained_models[-2], device, X_test, Y_test)   # Evaluate model on initial testing data
			# print("6) Accuracy on initial testing data (X_test, Y_test)", acc_test_init_orig)
			print("6)", acc_test_init_orig)


			acc_retraining_set_training_data= i7 = test(trained_models[-2], device, retraining_set.train_X, retraining_set.train_Y) # Evaluate model on retraining training data
			# print("7) Accuracy on retraining set training data = ", acc_retraining_set_training_data)
			print("7) ", acc_retraining_set_training_data)
			

			acc_retraining_set_testing_data= i8  = test(trained_models[-2], device, retraining_set.test_X, retraining_set.test_Y)   # Evaluate model on retraining testing data
			# print("8) Accuracy on retraining set testing data = ", acc_retraining_set_testing_data)
			print("8) ", acc_retraining_set_testing_data)
			...
			acc_train_init_orig = i9 = test(retrained_model, device, X_train, Y_train) # Evaluate model on initial training data
			# print("9) Accuracy on initial training data (X_train, Y_train)", acc_train_init_orig)
			print("9)", acc_train_init_orig)


			acc_test_init_orig = i10 =test(retrained_model, device, X_test, Y_test)   # Evaluate model on initial testing data
			# print("10) Accuracy on initial testing data (X_test, Y_test)", acc_test_init_orig)
			print("10)", acc_test_init_orig)
			

			acc_retraining_set_training_data = i11 = test(retrained_model, device, retraining_set.train_X, retraining_set.train_Y) # Evaluate model on retraining training data
			# print("11) Accuracy on retraining set training data = ", acc_retraining_set_training_data)
			print("11)", acc_retraining_set_training_data)
			

			acc_retraining_set_testing_data = i12 = test(retrained_model, device, retraining_set.test_X, retraining_set.test_Y)   # Evaluate model on retraining testing data
			# print("12) Accuracy on retraining set testing data = ", acc_retraining_set_testing_data)
			print("12) ", acc_retraining_set_testing_data)

			acc_accumilated_retraining_set_training_data = i13 = test(retrained_model, device, accumilated_retraining_set.train_X, accumilated_retraining_set.train_Y) # Evaluate model on retraining training data
			# print("13) Accuracy on retraining set training data = ", acc_accumilated_retraining_set_training_data)
			print("13)  ", acc_accumilated_retraining_set_training_data)
			
			acc_accumilated_retraining_set_testing_data = i14 = test(retrained_model, device, accumilated_retraining_set.test_X, accumilated_retraining_set.test_Y)   # Evaluate model on retraining testing data
			# print("14) Accuracy on retraining set testing data = ", acc_accumilated_retraining_set_testing_data)
			print("14) ", acc_accumilated_retraining_set_testing_data)
