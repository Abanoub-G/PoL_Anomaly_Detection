...
def create_unseen_scenarios(x_train,t_train,train_digit_index, index):
 ...
 index_list = train_digit_index[index][0:6215]
 ...
 second_x_train = np.reshape(second_x_train[0:200],(len(second_x_train[0:200]),1,28,28))
 second_t_train = np.reshape(second_t_train[0:200],(len(second_t_train[0:200]),))     
...
def train_ewc(model, device, task_id, x_train, t_train, optimizer, epoch, EWC_ON):
    model.train()
    ...
    for start in range(0, len(t_train)-1, 100):
      ...
      if EWC_ON:
          ...
          for task in range(task_id):
            ...
            second_part = second_part + task_part* ewc_lambda[task]
...
for epoch in range(0, 20):
   train_ewc(model, device, 0, x_train, t_train, optimizer, epoch, EWC_ON)
...
for epoch in range(0, 2):
   train_ewc(model, device, 1, second_x_train, second_t_train, optimizer, epoch, EWC_ON)
# Testing
for i,dataset in enumerate(test_digit):
 print("Test on Digit", i)
 test(model, device, test_digit[i], test_label[i],"Retrained_model")     
#
